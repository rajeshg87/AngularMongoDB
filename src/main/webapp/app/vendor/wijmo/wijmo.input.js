    ...
    (function (input) {
        ...
        var ComboBox = (function (_super) {
            ...
            ComboBox.prototype._keydown = function (e) {
                ...
                // handle key up/down keys to move to the next/previous items (TFS 153089)
                switch (e.keyCode) {
                    case wijmo.Key.Up:
                    case wijmo.Key.Down:
                        var index = this.selectedIndex + (e.keyCode == wijmo.Key.Up ? -1 : +1);
                        this.selectedIndex = (index + cv.items.length) % cv.items.length;
                        this._tbx.select();
                        e.preventDefault();
                        break;
                }
    ...
    (function (input) {
        ...
        var Popup = (function (_super) {
            ...
            function Popup(element, options) {
                ...
                this.addEventListener(host, 'keydown', function (e) {
                    if (!e.defaultPrevented) {
                        ...
                        if (e.keyCode == wijmo.Key.Enter) {
                            // compute dialog result
                            var result = _this.dialogResultEnter;
                            if (!result && _this.hostElement.querySelector('button[type=submit]')) {
                                result = 'submit';
                            }
                            // if we have a result, validate and hide
                            if (result) {
                                e.preventDefault();
                ...
                this.addEventListener(host, 'click', function (e) {
                    if (e.target instanceof HTMLElement) {
                        // check for elements with the 'wj-hide' class
            ...
            Popup.prototype.onLostFocus = function (e) {
                var _this = this;
                setTimeout(function () {
                    if (!_this.containsFocus() && wijmo.getActiveElement() != document.body) {
                        if (_this.isVisible && (_this._hideTrigger & PopupTrigger.Blur)) {
                            _this.hide();
                        }
                    }
                }, 100);
