...
(function (wijmo) {
    ...
    function moveFocus(parent, offset) {
        // build array of focusable elements
        var inputs = parent.querySelectorAll('input,select,textarea,button,a'), focusable = [];
...
(function (wijmo) {
    ...
    var Globalize = (function () {
        ...
        Globalize.formatNumber = function (value, format, trim, truncate) {
            ...
            // p: percentage
            if (f1 == 'p') {
                value = wijmo.toFixed(value * 100, prec, truncate); // TFS 147585
            }
            // truncate value
            if (truncate) {
        ...
        Globalize.parseDate = function (value, format) {
            ...
            var cal = wijmo.culture.Globalize.calendar, cjk = Globalize._CJK, rxv = new RegExp('(\\' + cal['/'] + ')|(\\' + cal[':'] + ')|' +
                '(\\d+)|' +
                '([' + cjk + '\\.]{2,})|' +
                '([' + cjk + ']+)', // strings with no dots
            'gi'), vparts = value.match(rxv), fparts = Globalize._parseDateFormat(format), offset = 0, year = -1, month = 0, day = 1, hour = 0, min = 0, tzm = 0, sec = 0, ms = 0, era = -1, hasDayName, hasDay, hasQuarter, hasMonth, fiscalFmt;
        ...
        Control.prototype.applyTemplate = function (classNames, template, parts, namePart) {
            ...
            if (input && atts) {
                for (var i = 0; i < atts.length; i++) {
                    if (atts[i].name.match(/name|autofocus|minlength|maxlength|pattern/i)) {
            ...
            if (input) {
                var evtChange = document.createEvent('HTMLEvents'), orgVal = '';
                ...
                for (var e = _this._e; e; e = e.parentElement) {
                    ...
                    if (ctl) {
                        ...
                        if (focus != ctl._focus) {
                            ...
                            wijmo.toggleClass(e, 'wj-state-focused', focus);
                        }
                    }
                }
...
(function (wijmo) {
    ...
    (function (collections) {
        ...
        var CollectionView = (function () {
             ...
             */
            CollectionView.prototype.getAggregate = function (aggType, binding) {
            ...
            CollectionView.prototype.onCollectionChanged = function (e) {
                if (e === void 0) { e = collections.NotifyCollectionChangedEventArgs.reset; }
                // track changes applied to items outside of editItem/commitEdit blocks
                if (!this._committing &&
             ...
             */
            CollectionView.prototype.onSourceCollectionChanged = function () {
                this.sourceCollectionChanged.raise(this);
            };
...
(function (wijmo) {
    ...
    var Clipboard = (function () {
        ...
        Clipboard._copyPasteInternal = function (textOrCallback) {
            ...
            // (body may not be focusable when modal dialogs are used...)
            var parent = wijmo.closest(parent, 'div') || document.body;
...
(function (wijmo) {
    ...
    function showPopup(popup, ref, above, fadeIn, copyStyles) {
        ...
        var rcp = ref == null ? new wijmo.Rect(0, 0, 0, 0)
            : parent == document.body ? new wijmo.Rect(-pageXOffset, -pageYOffset, 0, 0)
